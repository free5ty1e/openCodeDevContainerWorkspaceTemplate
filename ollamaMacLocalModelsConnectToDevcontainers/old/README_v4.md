...
| Document | Purpose |
|--------|--------|
| `ollama_models_64gb_reference_pretty.md` | Human‑readable model catalog |
| `ollama_models_64gb_csv.md` | Script‑friendly model matrix |
| `ollama_64gb_safe_defaults.md` | Known‑good conservative defaults |
| `ollama_64gb_ctx_cheatsheet.md` | Context sizing guidance by model |
| `ollama_64gb_devcontainer_safe.md` | Models safe for Docker / devcontainers |
| `ollama_model_licenses.md` | High‑level license notes |
...
| Document | Purpose |
|--------|--------|
| `opencode_ollama_master_wiki_v4.md` | Primary OpenCode reference (install, config, strategy) |
| `opencode_ollama_faq_v1.md` | FAQ and design decisions |
| `opencode_ollama_64gb_recommendations.md` | OpenCode‑specific model guidance |
...
| Document | Purpose |
|--------|--------|
| `ollama_opencode_alternatives.md` | Step‑by‑step alternatives to OpenCode (CLI, TUI, WebUI) |
| `quick-decision-matrix.md` | One‑page guide: which tool to use *right now* |
...
| Document | Use when |
|--------|----------|
| `Networking-Scenarios.md` | Choosing the correct supported network model |
| `lan_exposure.md` | LAN exposure when inbound access *is allowed* |
| `ollama_firewall_locked_access.md` | Policy‑safe access when inbound firewall rules are blocked |
| `ollama_firewall_locked_windows_devcontainer.md` | **Concrete runbook:** Windows devcontainer → macOS Ollama via SSH tunnel |
...
| Script | Purpose |
|------|---------|
| `install-models-with-ctx-v2.sh` | **Recommended**: pull models, create `-ctxNNNN` variants, generate `opencode.json.generated` |
| `install-models-with-ctx.sh` | Legacy v1 script (kept for reference) |
