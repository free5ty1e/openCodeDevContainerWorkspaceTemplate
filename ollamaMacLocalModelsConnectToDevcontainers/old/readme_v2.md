...
Good for refactors, multi‑file context, and long chat history.
...
Expect higher RAM pressure; monitor with `ollama ps`.

---

### Keep DeepSeek fast and stable

DeepSeek‑R1 is KV‑cache heavy. To override its default explicitly:
...
Useful for logs, transcripts, and large text inputs using 3B–8B models.

---

## Generated OpenCode config

If `opencode.json.generated` does not exist, the script creates it:

- Never overwrites your real config
- Uses the created `-ctxNNNN` variants
- Sets a sensible default, fallback, and vision model

**Recommended workflow**: copy the relevant sections into your real `opencode.json`.
