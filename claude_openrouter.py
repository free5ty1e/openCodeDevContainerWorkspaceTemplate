#!/usr/bin/env python3
"""
OpenRouter → Claude Code Bridge Script

Bridges OpenRouter models with the Claude Code CLI via a local litellm proxy.
"""
import argparse
import os
import sys
...
CACHE_FILE = os.path.expanduser("~/.openrouter_api_key_cache")
MODEL_CACHE_FILE = os.path.expanduser("~/.claude_openrouter_last_model")
FAVORITES_CACHE_FILE = os.path.expanduser("~/.claude_openrouter_favorites")
CONTEXT_CACHE_FILE = os.path.expanduser("~/.claude_openrouter_last_context")
CONTEXT_WINDOW_CACHE_DIR = os.path.expanduser("~/.claude_openrouter_context_windows")
STATUSLINE_MODE_CACHE_FILE = os.path.expanduser("~/.claude_openrouter_statusline_mode")

PROXY_PORT = 4503
PROXY_MASTER_KEY = "sk-openrouter-bridge"
CONFIG_DIR = os.path.expanduser("~/.claude_openrouter")
...
PROVIDER_INDICATOR = "openrouter"

# ─── Provider-Specific Functions ──────────────────────────────────────────────
def fetch_models(api_key):
    """Fetch the list of available models from the OpenRouter API."""
    try:
        data = http_get_json(OPENROUTER_API_URL, api_key)
        # OpenRouter returns data in a 'data' array
        raw = data.get("data", [])
        models = []
        for m in raw:
            model_id = m.get("id", "")
            if not model_id:
                continue
            # OpenRouter provides context length in the 'context_length' field
            context_window = m.get("context_length", 0)
            # Some models might use 'max_length' as fallback
            if context_window == 0:
                context_window = m.get("max_length", 0)
            models.append({
                "id": model_id,
                "context_window": context_window,
                # OpenRouter doesn't clearly separate free/paid in a simple boolean
                # We'll use the 'is_free' field if available, or check pricing
...
def display_and_select(standard, free, combined, args=None):
    ...
    if standard:
        ...
        for model_obj in standard:
            ...
            # Show free tier indicator
            free_indicator = " (Free)" if model_obj.get("is_free", False) else ""
            ctx_str = f" ({ctx:,} tokens)" if ctx > 0 else " (context unknown)"
            print(f"[{current_number}] {model_id}{ctx_str}{free_indicator}")
            current_number += 1
    if free:
        print("\n--- Free Tier Models ---")
...
def generate_litellm_config(selected_model, api_key, context_window=None):
    ...
    if context_window is None or context_window <= 0:
        ...
        if selected_model_data and selected_model_data.get("context_window", 0) > 0:
            ...
            # Fallback for known models (though OpenRouter API should provide this)
            CONTEXT_WINDOWS = {
                # Common models - values should come from API, but keep as fallback
                "anthropic/claude-3-opus": 200000,
                "anthropic/claude-3-sonnet": 200000,
                "anthropic/claude-3-haiku": 200000,
                "anthropic/claude-2.1": 200000,
                "anthropic/claude-2.0": 200000,
                "openai/gpt-4": 8192,
...
def launch_claude_with_model(selected_model, context_window, dangerously_skip_permissions=False, compaction_threshold=None):
    ...
    print("\n🚀 Launching Claude Code with selected OpenRouter model...")
...
def print_usage_notes(dangerously_skip_permissions=False, args=None):
    ...
    print("   This script bridges OpenRouter models with the Claude Code CLI.")
    print("   It fetches available OpenRouter models, lets you select one,")
    print("   runs a local litellm proxy (OpenRouter→Anthropic translation),")
    print("   then launches Claude Code configured to use that OpenRouter model.")
    print()
    print("🔑  API KEY SETUP:")
    print("   • Set OPENROUTER_API_KEY environment variable export")
    print("     OPENROUTER_API_KEY='sk-or-...'")
    print("   • Or run the script once - it will prompt and cache the key")
    print("     to ~/.openrouter_api_key_cache for future runs.")
    print()
    if dangerously_skip_permissions:
    ...
    print("   OpenRouter provides a unified API to access various LLM providers.")
    print("   A local litellm proxy translates between them so conversation")
    print("   and tool calls work against the OpenRouter model.")
    print()
...
def main():
    parser = argparse.ArgumentParser(
        description="Bridge OpenRouter models with the Claude Code CLI"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=False,
        help="Clear the cached OpenRouter API key and prompt again",
    )
    parser.add_argument(
        "--accept-all-defaults",
        action="store_true",
    ...
    api_key = get_and_cache_api_key(CACHE_FILE, "OPENROUTER_API_KEY", clear=args.clear_api_key)
    if api_key is None:
        print("🔑 OpenRouter API key not found.")
        try:
            api_key = input("   Please enter your OpenRouter API key (or press Enter for anonymous): ").strip()
        except EOFError:
    ...
    print("\n🔄 Fetching model list from OpenRouter API...")
    ...
    try:
        if not test_proxy_connection(PROXY_PORT, PROXY_MASTER_KEY, selected_model):
            ...
            print("   Check ~/.claude_openrouter/proxy.log for details.")
